...
public static void main(String[] args){
  ...
  while (!StdIn.isEmpty()) { //reading from input file
         String s = StdIn.readLine();
         if (s.contains("--")) { // if input is an edge
         	String[] from = s.split("--");
            String[] to = from[1].split(" \\[");
            int v = vertexMap.get(from[0].trim());
            int w = vertexMap.get(to[0].trim());
			String str=to[1].substring(0, to[1].length()-1);
            double we = Double.parseDouble(str);
        ...
        for (Edge edge : mst.edges()) {
            StdOut.println("from "+nameMap.get(edge.either()) +" to "+ nameMap.get(edge.other(edge.either())) + " :" + edge.weight()+" miles");
        }
